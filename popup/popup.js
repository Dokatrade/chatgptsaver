document.addEventListener('DOMContentLoaded', async () => {
    ...
    function transliterate(text) {
        ...
        return text.replace(/[а-яёА-ЯЁ]/g, ch => map[ch] || ch);
    }

    // Build an informative default filename: <title_latin>_<date>_<N>msg
    function buildFilename(title, msgCount) {
        const dateStr = new Date().toISOString().split('T')[0];
        const name = title ? transliterate(title) : 'chat';
        const countSuffix = msgCount > 0 ? `_${msgCount}msg` : '';
    ...
    try {
        ...
        if (isChatGPTTab(tab)) {
            ...
            if (response) {
                ...
                filenameInput.value = buildFilename(rawTitle, msgCount);
                chatInfoLoaded = true;
            }
        }
    } catch (error) {
    ...
    // Fallback: always have a value, never leave the field empty
    if (!chatInfoLoaded) {
        filenameInput.value = buildFilename('', 0);
    }
